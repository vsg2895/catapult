...
use App\Http\Resources\UserLeaderboardCollection;
...
class LeaderboardController extends Controller
{
     ...
     * )
     */
    public function index(UserLeaderboardRequest $request)
    {
        $users = User::query();
        ...
        if ($activityId) {
            $users = $users->whereHas('activities', function ($query) use ($activityId) {
                ...
                ->withCount(['tasksIsDone as tasks_count' => function ($query) use ($activityId) {
                    $query->whereRelation('task', fn ($q) => $q->where('activity_id', $activityId));
                }])
                ->withSum([
            ...
            $users = $users->whereHas('activities', fn ($query) => $query->active())
                ->withCount(['tasksIsDone as tasks_count' => function ($query) {
                    $query->whereRelation('task', fn ($q) => $q->whereNotNull('activity_id'));
                }])
        ...
        return response()->json(new UserLeaderboardCollection($users));
    }
